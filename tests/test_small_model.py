"""End-to-end adaptation on a random-weight EfficientNet-B0 at a 64 px input: no checkpoint, no network.

The model is the same timm ``efficientnet_b0`` architecture the pinned checkpoint loads into, with the
transform shrunk so a CPU step takes well under a second. These tests exercise the real prediction,
zero-shot, fine-tuning, evaluation, adapter export and adapter reload code paths. They say nothing
...
from efficientnet_classification_pipeline import (  # noqa: E402
    ARTIFACT_FORMAT,
    IMAGENET_GROUPS,
    MODEL_ID,
    MODEL_KEY,
...
from efficientnet_classification_pipeline import pipeline as pipeline_module  # noqa: E402
...
def _imagenet_pipeline(seed: int = 0) -> EfficientNetPipeline:
    torch.manual_seed(seed)
    return EfficientNetPipeline.from_components(
        build_model(), class_names=imagenet_labels(), transform=_transform()
    )


def _adapter_pipeline(seed: int = 0, class_names=SAMPLE_CLASSES) -> EfficientNetPipeline:
...
def test_build_model_matches_the_published_parameter_count():
    assert sum(p.numel() for p in build_model().parameters()) == 5_288_548  # the Hub card's "Params (M): 5.3"
    assert pipeline_module._backbone_prefixes(build_model())[0] == "conv_stem."
    assert f"{pipeline_module.HEAD_MODULE}." not in pipeline_module._backbone_prefixes(build_model())
