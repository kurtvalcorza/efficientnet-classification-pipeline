...
from efficientnet_classification_pipeline import pipeline as pipeline_module
...
def test_identity_constants_agree_with_the_committed_manifest():
    ...
    assert MODEL_ID == "timm/efficientnet_b0.ra_in1k" == manifest["modelId"]
    assert manifest["revision"] == pipeline_module.MODEL_REVISION
    assert is_pinned() == bool(HEX40.match(pipeline_module.MODEL_REVISION))
    assert DEFAULT_WEIGHTS_DIR == SNAPSHOT and ARTIFACT_FORMAT == "efficientnet-adapter-v1"
...
def test_committed_config_matches_the_manifest_size_and_timm_pretrained_cfg():
    ...
    assert (
        ...
        and config["pretrained_cfg"]["classifier"] == pipeline_module.HEAD_MODULE
    )
    timm = pytest.importorskip("timm")
    builtin = timm.create_model("efficientnet_b0.ra_in1k", pretrained=False).pretrained_cfg
...
def test_unpinned_package_refuses_every_weight_operation(tmp_path, monkeypatch):
    ...
    for call in (
        ...
        lambda: EfficientNetPipeline.from_pretrained(weights_dir=tmp_path),
    ):
