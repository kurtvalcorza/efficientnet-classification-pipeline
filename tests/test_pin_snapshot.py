...
FILES = {
    "README.md": b"# card\n",
    "config.json": b'{"architecture": "efficientnet_b0", "pretrained_cfg": {"tag": "ra_in1k"}}',
...
def test_pin_writes_commit_digests_and_module_revision(tmp_path):
    ...
    manifest = json.loads((root / "weights/efficientnet-b0-ra-in1k/dimer-base-manifest.json").read_text())
    ...
    module = (root / "src/efficientnet_classification_pipeline/pipeline.py").read_text()
    assert f'MODEL_REVISION = "{COMMIT}"' in module


def test_pin_refuses_a_digest_that_disagrees_with_the_hub(tmp_path):
    root = _copy_repo(tmp_path)
    before = (root / "weights/efficientnet-b0-ra-in1k/dimer-base-manifest.json").read_text()
    module_before = (root / "src/efficientnet_classification_pipeline/pipeline.py").read_text()
    ...
    assert (root / "weights/efficientnet-b0-ra-in1k/dimer-base-manifest.json").read_text() == before
    assert (root / "src/efficientnet_classification_pipeline/pipeline.py").read_text() == module_before
