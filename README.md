...
1. Run `python tools/pin_snapshot.py`. It resolves `main` to a commit, downloads the three manifest files at that commit into `weights/efficientnet-b0-ra-in1k/`, checks each LFS file against the Hub's SHA-256, and writes the commit and digests into the manifest and `MODEL_REVISION`.
