...
TEMPLATE = {
    "package": "efficientnet_classification_pipeline",
    "repo_name": "efficientnet-classification-pipeline",
    "stem": "efficientnet_classification",
    "notebook_name": "efficientnet_classification_colab.ipynb",
    "profile": "E2E",
    "mode": "GUIDED",
    "pipeline_class": "EfficientNetPipeline",
    "weights_key": "efficientnet-b0-ra-in1k",
    "modules": [
        "data.py",
        "pipeline.py",
    ],
    ...
    "title": "EfficientNet-B0 (timm, ImageNet-1k) — DIMER image classification and bounded fine-tuning (standalone)",
    "badges": [
        (
            "GitHub",
            "https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white",
            "https://github.com/kurtvalcorza/efficientnet-classification-pipeline",
        ),
        (
            "Open In Colab",
            "https://colab.research.google.com/assets/colab-badge.svg",
            "https://colab.research.google.com/github/kurtvalcorza/efficientnet-classification-pipeline/blob/main/tutorials/efficientnet_classification_colab.ipynb",
        ),
        (
            "Hugging Face",
            "https://img.shields.io/badge/%F0%9F%A4%97%20Hugging%20Face-timm%2Fefficientnet__b0.ra__in1k-ffcc4d?style=flat",
        ...
        (
            "arXiv",
            "https://img.shields.io/badge/arXiv-1905.11946-b31b1b.svg",
            "https://arxiv.org/abs/1905.11946",
        ),
        (
            "License",
            "https://img.shields.io/badge/License-Apache--2.0-green.svg",
            "https://github.com/kurtvalcorza/efficientnet-classification-pipeline/blob/main/LICENSE",
        ),
    ],
    "capability": "ImageNet-1k image classification with timm's EfficientNet-B0 (`ra_in1k` weights), and a bounded fine-tune that replaces the 1000-class head with one for your own classes, compares it on a held-out split with the majority-class and zero-shot ImageNet baselines, and exports a reloadable SafeTensors adapter",
    "intro": (
        "EfficientNet-B0 is the base model of the EfficientNet family: a convolutional network built from mobile inverted-bottleneck (MBConv) "
        "blocks with squeeze-and-excitation, found by neural architecture search and then scaled in depth, width and resolution together. The "
        "`timm/efficientnet_b0.ra_in1k` checkpoint was trained on ImageNet-1k in `timm` with the RandAugment (`RA`) recipe. Its 1000-class head "
        "outputs a softmax over the ImageNet classes; the checkpoint's own preprocessing resizes the image so its shorter side is 256 px and "
        "centre-crops 224×224.\n\n"
    ...
    "cells": [
        ...
        # ---------------------------------------------------------------- 7. Re-head & baseline
        {
            "md": (
                ...
                "(`classifier`, 1280 features to 1000 classes) with a new 1280-to-2 layer initialised under `SEED`. Every other weight keeps its "
                "ImageNet value.\n\n"
            ...
            "code": (
                "adapter = EfficientNetPipeline.from_pretrained(weights_dir=WEIGHTS_DIR, class_names=SAMPLE_CLASSES, seed=SEED)\n"
        ...
        # ---------------------------------------------------------------- 8. Fine-tune
        {
            "md": (
                ...
                "every BatchNorm layer outside it is held in evaluation mode, so the backbone keeps both its weights and its running statistics. "
        ...
        # ---------------------------------------------------------------- 11. Export, reload & verify
        {
            "md": (
                ...
                "`save_artifact` writes `outputs/efficientnet_adapter.safetensors`: every tensor the fine-tune could change, plus a metadata header "
            ...
            "code": (
                "artifact_path = OUTPUTS / 'efficientnet_adapter.safetensors'\n"
                "descriptor = adapter.save_artifact(artifact_path, notes='EfficientNet-B0 CIFAR-10 frog/truck tutorial adapter')\n"
                "print(json.dumps(descriptor, indent=2))\n\n"
                "reloaded = EfficientNetPipeline.load_artifact(artifact_path, weights_dir=WEIGHTS_DIR)\n"
        ...
        # ---------------------------------------------------------------- 12. Outputs & provenance
        {
            "md": (
                ...
                "The cell writes:\n"
                "- `outputs/efficientnet_classification_input_manifest.json`\n"
                "- `outputs/efficientnet_classification_evaluation_report.json`\n"
                "- `outputs/efficientnet_classification_result.json` (identity, runtime versions, device, dataset provenance and manifest, split, "
                "baselines, fine-tuning configuration, held-out and unseen metrics, adapter descriptor and reload check)\n"
                "- `outputs/efficientnet_classification_predictions.csv`\n"
                "- `outputs/efficientnet_adapter.safetensors` (written in Section 11)"
            ),
            "code": (
                "import csv\n\n"
        ...
        # ---------------------------------------------------------------- 13. BYOD
        {
            ...
            "code": (
                ...
                "    byod_pipe = EfficientNetPipeline.from_pretrained(weights_dir=WEIGHTS_DIR, class_names=byod_names, seed=SEED)\n"
                ...
                "    byod_descriptor = byod_pipe.save_artifact(OUTPUTS / 'byod_efficientnet_adapter.safetensors', notes='BYOD adaptation adapter')\n"
                "    EfficientNetPipeline.load_artifact(OUTPUTS / 'byod_efficientnet_adapter.safetensors', weights_dir=WEIGHTS_DIR)\n"
                "    print('BYOD adapter exported and reloaded:', byod_descriptor['sha256'][:16])\n"
                "else:\n"
                "    print('BYOD dataset branch is off; set USE_BYOD_DATASET = True to adapt EfficientNet-B0 on your own class folders.')"
            ),
        },
    ],
    "closing": (
        "## Interpretation and limits\n\n"
        "**What this notebook established, in this runtime.** The pinned `timm/efficientnet_b0.ra_in1k` snapshot was verified against a committed "
        ...
        "- Wightman, R., Touvron, H. and Jégou, H. (2021). *ResNet strikes back: An improved training procedure in timm.* [arXiv:2110.00476](https://arxiv.org/abs/2110.00476) — the `RA` recipe is its `B` recipe.\n"
        "- Hugging Face checkpoint: [timm/efficientnet_b0.ra_in1k](https://huggingface.co/timm/efficientnet_b0.ra_in1k) — Apache-2.0; code: [huggingface/pytorch-image-models](https://github.com/huggingface/pytorch-image-models).\n"
        ...
        "- Repository model card: https://github.com/kurtvalcorza/efficientnet-classification-pipeline/blob/main/MODEL_CARD.md\n"
        "- [`kurtvalcorza/efficientnet-classification-pipeline`](https://github.com/kurtvalcorza/efficientnet-classification-pipeline) — source repository for this pipeline."
    ),
}
