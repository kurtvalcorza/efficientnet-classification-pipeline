...
date_published: "2019-05"
date_published_source: "month of the EfficientNet paper (arXiv:1905.11946, submitted 2019-05-28); the date the timm ra_in1k weights were first published is not established by this repository"
---

# EfficientNet-B0 (timm, ImageNet-1k) — Image Classification with Bounded Fine-Tuning

[![Hugging Face](https://img.shields.io/badge/%F0%9F%A4%97%20Hugging%20Face-timm%2Fefficientnet__b0.ra__in1k-ffcc4d?style=flat)](https://huggingface.co/timm/efficientnet_b0.ra_in1k)
...
[![arXiv Paper](https://img.shields.io/badge/arXiv-1905.11946-b31b1b.svg)](https://arxiv.org/abs/1905.11946)
...
- **End-to-end classification and adaptation tutorial**:
  [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/kurtvalcorza/efficientnet-classification-pipeline/blob/main/tutorials/efficientnet_classification_colab.ipynb) [`efficientnet_classification_colab.ipynb`](https://github.com/kurtvalcorza/efficientnet-classification-pipeline/blob/main/tutorials/efficientnet_classification_colab.ipynb)
...
`timm/efficientnet_b0.ra_in1k` is an EfficientNet-B0 image classifier trained on ImageNet-1k in the `timm` library. EfficientNet (Tan and Le, arXiv:1905.11946) is a family of convolutional networks. B0 is the base network, found by neural architecture search and built from mobile inverted-bottleneck (MBConv) blocks with squeeze-and-excitation; B1 to B7 scale its depth, width and input resolution together. The upstream card reports 5.3M parameters, 0.4 GMACs and a 224×224 input. A test in this repository counts 5,288,548 parameters in the architecture it builds.

The `ra_in1k` tag names the training recipe. The upstream card describes it as the RandAugment (`RA`) recipe, published as recipe `B` in "ResNet strikes back" (Wightman et al., arXiv:2110.00476), with RMSProp in TensorFlow 1.0 behaviour, EMA weight averaging, and a stepped exponential learning-rate decay with warmup. The 1000-class head outputs a softmax over the ImageNet-1k classes. The checkpoint's preprocessing resizes the image with bicubic interpolation so its shorter side is 256 px, centre-crops 224×224 (`crop_pct` 0.875), and normalises with the ImageNet mean and standard deviation.

This repository adds gradient fine-tuning on a caller's labelled images. `from_pretrained(class_names=...)` replaces the 1280-to-1000 `classifier` layer with a new layer for the caller's classes. `finetune` then trains with cross-entropy, either the whole network (the default) or the new head alone with the backbone frozen.
...
The intended role is a small, fast reference classifier and a teaching baseline for transfer learning. EfficientNet-B0 is compact enough to fine-tune on a free GPU in minutes. A reader's own application can embed `EfficientNetPipeline` as a classifier or, with the head removed, as an image feature extractor.
...
- in the recorded tutorial run the adapted frog/truck head answered `truck` with score 1.0 for pure noise and split a blank image 0.509 / 0.491 between `truck` and `frog`, and the ImageNet head gave both CIFAR-10 frog sample images non-frog top-1 labels (`pick`, `platypus`).
...
The upstream card links timm's results tables for ImageNet accuracy. This repository does not reproduce any ImageNet figure.

Values measured by this repository (one run on Kaggle Tesla T4, 2026-09-26 UTC; exact notebook blob `fbcb57dc1362`, commit `9dce015`; one pass, no dispersion estimate):

- **ImageNet head on 4 sample images** (CIFAR-10, 32 px upsampled): the two frogs got `pick` (0.390) and `platypus` (0.121), with no frog class in either top-5; the trucks got `moving van` (0.536) and `passenger car` (0.247). `sample-sanity` group hit rate 0.25 at top-1 and 0.5 at top-5. This is a four-image check, not an ImageNet evaluation.
- **Held-out** (60 images, 30 per class, split seed 0 from 400 images), accuracy / balanced accuracy: majority class 0.500 / 0.500; zero-shot ImageNet mapping 0.9667 / 0.9667 (2 of 30 frogs missed); untrained two-class head 0.5167 / 0.5167; fine-tuned 1.000 / 1.000. The fine-tune is two images above the zero-shot mapping, within the resolution of a 60-image split, so it shows **no measurable gain** over zero-shot here.
- **Unseen** (60 images): fine-tuned 1.000 / 1.000.
- **Fine-tune:** full model, 4,010,110 parameters trained, 5 epochs, batch 16, AdamW lr 0.0001; epoch losses 0.4823, 0.1611, 0.0719, 0.0346, 0.0166.
...
- BatchNorm statistics, which update during full fine-tuning (timm builds this model with `drop_rate` 0.0, so there is no dropout);
...
- `EfficientNetPipeline.from_pretrained(device=None, weights_dir=None, allow_download=False, class_names=None, seed=20260925)`: stage (only with `allow_download=True`), verify, load; with `class_names`, replace the head deterministically under `seed`.
...
- `save_artifact(path, *, notes=None) -> dict`; `read_artifact_metadata(path) -> dict`; `apply_artifact(path)`; `load_artifact(path, *, weights_dir=None, device=None)`. The adapter format is `efficientnet-adapter-v1`.
...
- Wightman, Touvron and Jégou. ResNet strikes back: An improved training procedure in timm. 2021. https://arxiv.org/abs/2110.00476
...
- Upstream card: https://huggingface.co/timm/efficientnet_b0.ra_in1k
