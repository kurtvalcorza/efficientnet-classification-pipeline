...
- Weight format: SafeTensors (`model.safetensors`, 21,355,344 bytes as the Hub reported for `main` when this repository was built). The Hub repository also holds `pytorch_model.bin` (21,441,905 bytes), a pickle checkpoint with the same weights; it is not in the manifest and is never staged or loaded.
- Manifest: `weights/efficientnet-b0-ra-in1k/dimer-base-manifest.json` (3 files: `README.md`, `config.json`, `model.safetensors`; `totalBytes` 21360641). Every `sha256` is `null` until the pin tool runs.
- Committed copy: `config.json` (578 bytes) is committed as the Hub served it, so the offline tests can check the architecture, the tag and the preprocessing against timm's built-in configuration. The pin tool replaces it with the bytes downloaded at the pinned commit before hashing, so the digests always describe the pinned bytes.
